...
- The allocation checks are a hard requirement: `igrf`/`igrfd` called with preallocated `P`/`dP` matrices and `show_warnings = Val(false)` must remain allocation-free (this is why the warning is controlled by a compile-time `Val` flag — `@warn` allocates, and AllocCheck flags it even in a branch that is not taken at runtime). The `MVector`s used inside the IGRF kernel are stack allocated only because they do not escape the function; do not return or store them. Their `@inbounds` accesses are also essential: with `--check-bounds=yes` the bounds-error branch makes them escape to the heap, so the allocation checks are skipped when bounds checking is forced (`Pkg.test` forces it on Julia < 1.13; CI passes `check_bounds: 'auto'` to `julia-runtest`, which otherwise defaults to `yes`). To reproduce CI locally, run `julia --project=. -e 'using Pkg; Pkg.test(coverage = true, julia_args = ["--check-bounds=auto"])'` and delete the generated `*.cov` files afterward.
